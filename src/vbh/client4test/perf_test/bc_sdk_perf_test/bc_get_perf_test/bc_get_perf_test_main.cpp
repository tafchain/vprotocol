...
CDscString* g_transKey; //存放成功的交易个数
...
int main(int argc, char* argv[])
{
	...
	g_transKey = new CDscString[g_nUserNum];
...
void DoTrade(char* pUserKey, size_t nUserKeyLen, 
	char* pCryptUserKey, size_t nCryptUserKeyLen)
{
	CTestTradeUserKey userKey;
...
void DoQueryTrans(char* pUserKey, size_t nUserKeyLen, 
	char* pCryptUserKey, size_t nCryptUserKeyLen, 
	char* pTransKey, size_t nTransKeyLen)
{
	//查询
...
void CTestRegUserMsgCallback::OnRegistUserRsp(VBH_SDK::CRegistUserRsp& rRegistUserRsp)
{
	...
	else
	{
		...
		else
		{
			...
			{
				const ACE_UINT32 n = g_nUserNum - 2;
...
void CTestOpUserMsgCallback::OnLaunchBcTransRsp(VBH_SDK::CSetBcUserInfoRsp& rSetUserInfoRsp)
{
	...
	else
	{
		...
		{
			ACE_UINT64 index = userKey.IDNumber - g_nBaseIDNumber;
			CDscString& strCrypt = g_cryptUserKey[index];
			CDscString& strTransKey = g_transKey[index];

			strTransKey.assign(rSetUserInfoRsp.m_pTransKey, rSetUserInfoRsp.m_nTransKeyLen);

			DoQueryTrans(rSetUserInfoRsp.m_pUserKey, rSetUserInfoRsp.m_nUserKeyLen, strCrypt.data(), strCrypt.size(), 
				strTransKey.data(), strTransKey.size());
...
{
	if (rGetTransInfoRsp.m_nReturnCode)
	{
		printf("OnGetBcTransInfoRsp() report error:%d\n", rGetTransInfoRsp.m_nReturnCode);
	}

	CTestTradeUserKey userKey;
	if (DSC::Decode(userKey, rGetTransInfoRsp.m_pUserKey, rGetTransInfoRsp.m_nUserKeyLen))
	{
		printf("OnRegistUserRsp(), decode user key error\n");
	}
	else
	{
		...
		strTransKey.assign(rGetTransInfoRsp.m_pTransKey, rGetTransInfoRsp.m_nTransKeyLen);

		DoQueryTrans(rGetTransInfoRsp.m_pUserKey, rGetTransInfoRsp.m_nUserKeyLen, strCrypt.data(), strCrypt.size(),
			strTransKey.data(), strTransKey.size());
...
void CTestRegUserMsgCallback::OnAbnormal(void)
{
	printf("CTestRegUserMsgCallback::OnAbnormal(void)\n");
}

void CTestRegUserMsgCallback::OnExit(void)
{
	g_bExit = true;
}
